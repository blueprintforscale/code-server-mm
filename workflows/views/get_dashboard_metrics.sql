CREATE OR REPLACE FUNCTION public.get_dashboard_metrics(p_start date DEFAULT (CURRENT_DATE - '30 days'::interval), p_end date DEFAULT CURRENT_DATE)
 RETURNS TABLE(customer_id bigint, client_name text, ads_manager text, inspection_type text, budget numeric, months_in_program integer, start_date date, field_mgmt text, timezone text, quality_leads integer, prior_quality_leads integer, actual_quality_leads integer, prior_actual_quality_leads integer, spam_contacts integer, lead_volume_change numeric, ad_spend numeric, all_time_spend numeric, cpl numeric, total_calls integer, spam_rate numeric, abandoned_rate numeric, days_since_lead integer, biz_hour_calls integer, biz_hour_answered integer, call_answer_rate numeric, hcp_insp_booked integer, hcp_closed_rev numeric, hcp_open_est_rev numeric, hcp_approved_no_inv numeric, jobber_insp_booked integer, jobber_closed_rev numeric, jobber_open_est_rev numeric, total_insp_booked integer, total_closed_rev numeric, total_open_est_rev numeric, approved_no_inv numeric, on_cal_14d integer, on_cal_total integer, lsa_spend numeric, lsa_leads integer, insp_booked_pct numeric, roas numeric, period_potential_roas numeric, guarantee numeric, all_time_rev numeric, trailing_6mo_roas numeric, trailing_6mo_potential_roas numeric, trailing_3mo_roas numeric, trailing_3mo_potential_roas numeric, prior_cpl numeric, risk_override text)
 LANGUAGE sql
 STABLE
AS $function$
