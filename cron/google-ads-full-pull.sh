...
claude -p --dangerously-skip-permissions \
  "Run the full Google Ads ETL pipeline including search terms: /Users/bp/data-pipeline/.venv/bin/python3 /Users/bp/data-pipeline/scripts/pull_google_ads.py --days 1

If the pipeline succeeds with no errors, output ONLY: SUCCESS - no further action needed.

If there are any errors or failures, post a message to the Slack #general channel using the Slack MCP tool summarizing what went wrong, including the error output. Start the Slack message with '🔴 Google Ads Full Pull Failed' followed by the error details." \
  >> "$LOGFILE" 2>&1
